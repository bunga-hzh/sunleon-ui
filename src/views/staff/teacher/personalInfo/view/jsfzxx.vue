...
import { option } from "@/views/staff/teacher/teacherInfo/option/jsfzxx";
...
export default {
  ...
  methods: {
    async getObj() {
      const { data: res } = await fetchList("jsfzxx", {
        current: 1,
        size: 1,
      ...
      this.obj = {
        ...res.data.records[0],
        jszgzsc: validatenull(res.data.records[0].jszgzsc)
          ? undefined
          : splitUploadData(res.data.records[0].jszgzsc),
      };
    },
    submit(form, loading) {
      ...
      setTimeout(async () => {
        ...
        let obj = {
          ...form,
          staffId: this.userInfo.userId,
          jszgzsc: validatenull(form.jszgzsc)
            ? undefined
            : form.jszgzsc[0].value,
        };
        if (this.id) {
          //编辑
          const { data: res } = await putObj("jsfzxx", { ...obj, id: this.id });
          if (res.code !== 0) return this.$message.error(res.msg);
          this.$message.success("保存成功!");
        } else {
          //添加
          const { data: res } = await addObj("jsfzxx", obj);
          if (res.code !== 0) return this.$message.error(res.msg);
          this.id = res.data;

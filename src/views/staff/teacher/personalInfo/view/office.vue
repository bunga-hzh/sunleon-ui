...
import { option } from "@/views/staff/teacher/teacherInfo/option/office";
...
export default {
  ...
  methods: {
    async getObj() {
      const { data: res } = await fetchList("office", {
        current: 1,
        size: 1,
      ...
      this.obj = {
        ...res.data.records[0],
        jyclsc: validatenull(res.data.records[0].jyclsc)
          ? undefined
          : splitUploadData(res.data.records[0].jyclsc),
      };
    },
    submit(form, loading) {
      ...
      setTimeout(async () => {
        ...
        if (this.id) {
          //编辑
          const { data: res } = await putObj("office", { ...obj, id: this.id });
          if (res.code !== 0) return this.$message.error(res.msg);

...
import { option } from "../option/salary";
...
export default {
  ...
  watch: {
    ...
    activeName(newValue) {
      ...
      if (newValue == "salary") {
        ...
        this.obj = {
          ...this.formObj,
          xjdjfj: validatenull(this.formObj.xjdjfj)
            ? undefined
            : splitUploadData(this.formObj.xjdjfj),
        };
      }
    },
    formObj(newValue) {
  ...
  methods: {
    submit(form, loading) {
      ...
      let obj = {
        ...form,
        xjdjfj: validatenull(form.xjdjfj) ? undefined : form.xjdjfj[0].value,
      };
      setTimeout(async () => {
        loading();
        if (this.id) {
          //编辑
          const { data: res } = await edit("salary", { ...obj, id: this.id });
          if (res.code !== 0) return this.$message.error(res.msg);
          this.$message.success("保存成功!");
        } else {
          //添加
          const { data: res } = await add("salary", {
            ...obj,

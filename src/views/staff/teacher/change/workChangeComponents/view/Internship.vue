...
<script>
import { option } from "../option/Internship";
...
export default {
  ...
  methods: {
    ...
    async rowSave(form, done, loading) {
      ...
      const { data: res } = await addObj("ywglsxqbhg", obj);
      if (res.code !== 0) return this.$message.error(res.msg);
    ...
    async rowUpdate(form, index, done, loading) {
      ...
      const { data: res } = await putObj("ywglsxqbhg", obj);
      if (res.code !== 0) return this.$message.error(res.msg);
      done(obj);
    ...
    rowDel(form, index) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(async () => {
          const { data: res } = await delObj("ywglsxqbhg", form.id);
          if (res.code !== 0)

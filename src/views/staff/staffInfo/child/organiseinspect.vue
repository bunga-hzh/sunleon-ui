<template>
  <basic-container>
    ...
    <excel-upload ref="excelUpload"
                  title="用户信息导入"
                  url="/staff/zzjgorganiseinspect/import"
...
<script>
import { option } from "@/views/staff/staffInfo/option/child/organiseinspect";
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      this.downBlobFile(
        "/staff/zzjgorganiseinspect/export",
        this.search,
        "教职工组织考察信息表.xlsx"
      );
    },
    ...
    async rowSave(form, done, loading) {
      const { data: res } = await addObj("organiseinspect", form);
      if (res.code !== 0) return this.$message.error(res.msg);

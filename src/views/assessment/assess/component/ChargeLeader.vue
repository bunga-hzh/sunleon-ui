...
export default {
  ...
  data() {
    return {
      yearId: null,
      yearName: null,
      assessType: ASSESS_TYPE.FGLDPZC,
      standards: [],
      middleUsers: [],
      loadingText: "",
      send: false,
  ...
  methods: {
    ...
    queryMiddleUsers() {
      this.$http
        .get("/assess/middleRangeUsers", {
          params: {
            userRangeId: this.$store.state.user.userRangeId,
          },
        })
        .then((res) => {
          this.middleUsers = res.data.data;
          this.rangeSize = this.middleUsers.length;
          if (this.rangeSize <= 0) {
            this.send = false;
          }
    ...
    save(state) {
      ...
      if (checkRes.length == 0) {
        let title = "确定要暂存对中层的评分吗？";
        if (state == 2) {
          title = "确定要提交对中层的评分吗？提交后将无法修改";
        }

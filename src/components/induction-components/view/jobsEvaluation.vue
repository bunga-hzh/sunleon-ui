<template>
  <div style="margin-top: 16px;">
      <avue-form ref="jobsEvaluationForm" v-model="data" :option="option" @submit="handleSubmit">
      </avue-form>
  </div>
</template>

<script>
import option from '../option/jobsEvaluationOption'
...
export default {
  ...
  methods:{
    //获取合同基本信息
    getContractInfo() {
      fetchListByType(this.id,17).then(res=>{
        // console.log();
        if(res.data.data){

<template>
  <div style="margin-top: 16px;">
      <avue-form ref="passportForm" v-model="data" :option="option" @submit="handleSubmit">
      </avue-form>
  </div>
</template>

<script>
import option from '../option/passportOption'
...
export default {
  ...
  methods:{
    //获取合同基本信息
    getContractInfo() {
      fetchListByType(this.id,18).then(res=>{
        // console.log();
        if(res.data.data){

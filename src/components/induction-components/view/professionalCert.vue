<template>
  <div style="margin-top: 16px;">
      <avue-form ref="professionalCertForm" v-model="data" :option="option" @submit="handleSubmit">
      </avue-form>
  </div>
</template>

<script>
import option from '../option/professionalCertOption'
...
export default {
  ...
  methods:{
    //获取合同基本信息
    getContractInfo() {
      fetchListByType(this.id,14).then(res=>{
        // console.log();
        if(res.data.data){

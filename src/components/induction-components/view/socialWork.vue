<template>
  <div style="margin-top: 16px;">
      <avue-form ref="socialWorkForm" v-model="data" :option="option" @submit="handleSubmit">
      </avue-form>
  </div>
</template>

<script>
import option from '../option/socialWorkOption'
...
export default {
  ...
  methods:{
    //获取合同基本信息
    getContractInfo() {
      fetchListByType(this.id,16).then(res=>{
        // console.log();
        if(res.data.data){

<template>
  <div style="margin-top: 16px;">
      <avue-form ref="schoolRoleForm" v-model="data" :option="option" @submit="handleSubmit">
      </avue-form>
  </div>
</template>

<script>
import option from '../option/schoolRoleOption'
...
export default {
  ...
  methods:{
    //获取合同基本信息
    getContractInfo() {
      fetchListByType(this.id,3).then(res=>{
        if(res.data.data){

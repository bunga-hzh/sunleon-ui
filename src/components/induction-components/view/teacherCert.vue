<template>
  <div style="margin-top: 16px;">
      <avue-form ref="teacherCertForm" v-model="data" :option="option" @submit="handleSubmit">
      </avue-form>
  </div>
</template>

<script>
import option from '../option/teacherCertOption'
...
export default {
  ...
  methods:{
    //获取合同基本信息
    getContractInfo() {
      fetchListByType(this.id,13).then(res=>{
        // console.log();
        if(res.data.data){

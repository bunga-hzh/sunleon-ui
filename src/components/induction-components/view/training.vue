<template>
  <div style="margin-top: 16px;">
      <avue-form ref="trainingForm" v-model="data" :option="option" @submit="handleSubmit">
      </avue-form>
  </div>
</template>

<script>
import option from '../option/trainingOption'
...
export default {
  ...
  methods:{
    //获取合同基本信息
    getContractInfo() {
      fetchListByType(this.id,12).then(res=>{
        // console.log();
        if(res.data.data){
